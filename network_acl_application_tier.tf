...
resource "aws_network_acl_rule" "application_https_ingress_from_cidr_blocks" {
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_https_ingress_from_cidr_blocks[count.index]
...
resource "aws_network_acl_rule" "application_http_ingress_from_cidr_blocks" {
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_http_ingress_from_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}
...
resource "aws_network_acl_rule" "application_all_ingress_from_cidr_blocks" {
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_all_ingress_from_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}
...
resource "aws_network_acl_rule" "application_ssh_ingress_from_cidr_blocks" {
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_ssh_ingress_from_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}
...
resource "aws_network_acl_rule" "application_ephemeral_ingress_from_cidr_blocks" {
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_ephemeral_ingress_from_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}
...
resource "aws_network_acl_rule" "application_postgresql_egress_to_cidr_blocks" {
  ...
  rule_number    = 100 + (10 * count.index)
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_postgresql_egress_to_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}
...
resource "aws_network_acl_rule" "application_all_egress_to_cidr_blocks" {
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_all_egress_to_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}


locals {
  network_acl_application_ephemeral_egress_to_cidr_blocks = flatten([
    "0.0.0.0/0"
  ])
}

// https://www.terraform.io/docs/providers/aws/r/network_acl_rule.html
resource "aws_network_acl_rule" "application_ephemeral_egress_to_cidr_blocks" {
  count          = length(local.network_acl_application_ephemeral_egress_to_cidr_blocks)
  egress         = true
  from_port      = 1024
  to_port        = 65535
  protocol       = "tcp"
  ...
  rule_action    = "allow"
  cidr_block     = local.network_acl_application_ephemeral_egress_to_cidr_blocks[count.index]
  network_acl_id = aws_network_acl.application_tier.id
}

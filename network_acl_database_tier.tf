...
resource "aws_network_acl_rule" "database_ssh_ingress_from_cidr_blocks" {
    ...
    protocol = "tcp"
    rule_number = "${100
    + (10 * length(local.network_acl_database_postgresql_ingress_from_cidr_blocks))
    + (10 * count.index)}"
    rule_action = "allow"
...
resource "aws_network_acl_rule" "database_all_ingress_from_cidr_blocks" {
    ...
    rule_number = "${100
    + (10 * length(local.network_acl_database_postgresql_ingress_from_cidr_blocks))
    + (10 * length(local.network_acl_database_ssh_ingress_from_cidr_blocks))
    + (10 * count.index)}"
    rule_action = "allow"
...
resource "aws_network_acl_rule" "database_ephemeral_ingress_from_cidr_blocks" {
    ...
    + (10 * length(local.network_acl_database_postgresql_ingress_from_cidr_blocks))
    + (10 * length(local.network_acl_database_ssh_ingress_from_cidr_blocks))
    + (10 * length(local.network_acl_database_all_ingress_from_cidr_blocks))
    + (10 * count.index)}"
    rule_action = "allow"
...
locals {
    network_acl_database_ephemeral_egress_from_cidr_blocks = "${flatten([
        "0.0.0.0/0",
        local.application_tier_cidr_block,
        local.database_tier_cidr_block,
        local.management_tier_cidr_block
...
resource "aws_network_acl_rule" "database_ephemeral_egress_from_cidr_blocks" {
    ...
    protocol = "tcp"
    rule_number = "${100
    + (10 * length(local.network_acl_database_all_egress_to_cidr_blocks))
    + (10 * count.index)}"
    rule_action = "allow"
